...
class UserRecoveryForm extends CFormModel
{
    ...
    public function rules()
    {
        return array(
            ...
            array('login_or_email', 'match', 'pattern' => '/[A-Za-z0-9@.-_\s,]+$/u', 'message' => UserModule::t("Incorrect symbols (A-z0-9).")),
    ...
    public function checkexists($attribute, $params)
    {
        if (!$this->hasErrors())  // we only want to authenticate when no input errors
        {
            if (strpos($this->login_or_email, "@")) {
                ...
                $user = User::model()->findByAttributes(array('name' => $this->login_or_email));
                if ($user)
                    $this->user_id = $user->id;
            }
            ...
            if ($user === null)
                if (strpos($this->login_or_email, "@")) {
                    $this->addError("login_or_email", UserModule::t("Неправильный email адрес"));
                } else {
                    $this->addError("login_or_email", UserModule::t("Имя введено неправильно."));
                }
        }
    }
