...
<div class="well">
    <div style="min-width:800px;">
        <div>
            <? echo CHtml::ajaxLink('Список неоплаченных заказов', '/admin/partner/id/' . $model->id . '/getOrderDebtList', array('update' => '#debtlist')); ?>
            &nbsp&nbsp&nbsp&nbsp&nbsp&nbsp
            <a href="/admin/partner/id/<?= $model->id; ?>/removeOrderDebt/"
               onclick="return confirm('Обнулить задолженость?') ? true : false;">Снять задолженность</a>
        </div>
        <br><br>
            ...
            <script type="text/javascript">
                function drawVisualization() {
                    ...
                    var control = new google.visualization.ControlWrapper({
                        ...
                        <?/*if(count($arr_keys)>2){?>'state': {'range': {'start': new Date(<?echo date('Y, m-1, d', strtotime($arr_keys[floor(count($arr_keys)/2)]));?>)}}<?}*/?>
                    });
                    ...
                    <? while($date <= $date_end){//идем от мин даты до макс с интервалом в 1 день?>
