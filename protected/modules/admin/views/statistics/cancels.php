...
<div class="table12">
    ...
    <a class="btn <? if ((isset($_GET['period']) && ($_GET['period'] == 'day')) || !isset($_GET['period'])) { ?>btn-primary<? } ?>"
       href="/admin/statistics/cancels?period=day<? if (!empty($_GET['partner_id'])) echo "&partner_id=" . $_GET['partner_id'] ?>">Дни</a>
    <a class="btn <? if (isset($_GET['period']) && $_GET['period'] == 'month') { ?>btn-primary<? } ?>"
       href="/admin/statistics/cancels?period=month<? if (!empty($_GET['partner_id'])) echo "&partner_id=" . $_GET['partner_id'] ?>">Месяцы</a>
    <a class="btn <? if (isset($_GET['period']) && $_GET['period'] == 'year') { ?>btn-primary<? } ?>"
       href="/admin/statistics/cancels?period=year<? if (!empty($_GET['partner_id'])) echo "&partner_id=" . $_GET['partner_id'] ?>">Года</a>
    <br><br>
...
<script>
    $('.relation_partner').change(function () {
        ...
        window.location = '/admin/statistics/cancels?' + kalendar + "partner_id=" + this.value;
    });
</script>