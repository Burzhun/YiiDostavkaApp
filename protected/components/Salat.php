...
class Salat
{
     ...
     * @author Khaled Al-Sham'aa <khaled@ar-php.org>
     */
    public function setLocation($l1 = 37.15861, $l2 = 36.20278, $altitude=300, $z = 4)
    {
    ...
    public function setConf($sch = 'Shafi', $sunriseArc = -0.833333, 
                            $ishaArc = -17, $fajrArc = -18, $view = 'Sunni')
    {
        $sch = ucfirst($sch);
    ...
    public function getPrayTime2()
    {
        ...
        // the object itself plus the length of that object's shadow at noon
            ...
            $n10 = sin(atan(1/(2 + tan(deg2rad($this->lat - $D))))) - sin(deg2rad($this->lat)) * sin(deg2rad($D));
        
        $Asr = $Dhuhr + (1/15) * rad2deg(acos($n / $d));
		$Asr10 = $Dhuhr + (1/15) * rad2deg(acos($n10 / $d));
        ...
        $times = array($Fajr, $Sunrise, $Dhuhr, $Asr, $Maghrib, $Isha, $Sunset, $Midnight, $Imsak, $Asr10);
